...
foreach ($test in ($currentConfig.Tests.Test | Sort-Object @{e={$_.order -as [int]}})) {
	...
	if($errorOccurredDuringTest -eq $null) {
    ...
    switch($test.Type) {
        ...
        "PSScript" {
            ...
            foreach ($param in $test.Params.Param) {
				if($param.Name -eq "OutputFile") {
					$PSScriptOutputFile = $param.Value
				}
				$allPSArgs += "-" + $param.Name
				$allPSArgs += "'" + $param.Value + "'"
            }
			
            if($currentConfig.Debug -eq 1) {
                Log-Write -LogPath $sLogFile -LineValue "[DEBUG] - Generated Command : & $($test.FileLocation)" + [string]$allPSArgs
            }
			
            try {

				Invoke-Expression "$($test.FileLocation) $allPSArgs"
				
				if(($PSScriptOutputFile -ne $null) -and (Test-Path $PSScriptOutputFile) -ne $true) {
					throw "No output file found. Suspecting and error occurred during the execution of the external script."
				}

            }
            catch {
                # Discovering the full type name of an exception
                Log-Error -LogPath $sLogFile -ErrorDesc "$padding$($_.Exception.gettype().fullName)" -ExitGracefully $false
                Log-Error -LogPath $sLogFile -ErrorDesc "$padding$($_.Exception.message)" -ExitGracefully $false
